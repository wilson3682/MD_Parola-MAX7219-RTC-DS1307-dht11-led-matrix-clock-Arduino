...
// - DHT11 library (DHT11) found at https://github.com/markruys/arduino-DHT
//
...
void getDate(char *psz) {
  char  szBuf[10];
  RTC.readTime();
  //sprintf(psz, "%d %s %04d", RTC.dd, mon2str(RTC.mm, szBuf, sizeof(szBuf) - 1), RTC.yyyy); // <<<<<=============================  Europe date Format
  sprintf(psz, "%s %d %04d", mon2str(RTC.mm, szBuf, sizeof(szBuf) - 1), RTC.dd, RTC.yyyy);   // <<<<<=============================  US date Format
}
...
// Code for reading clock date days
...
// Code for reading clock date months
...
// Code for reading clock date years
...
void setup(void) {
  ...
  RTC.control(DS1307_12H, DS1307_OFF); // <<<<<============================= 24 hrs Clock Format
  RTC.control(DS1307_12H, DS1307_ON);  // <<<<<============================= 12 hrs Clock Format
  getTime(szTime);
}
